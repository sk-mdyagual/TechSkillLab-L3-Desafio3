...
class InvoiceCalculatorRefacTest {
    ...
    @Test
    void calcularTotalConImpuestoIva_retornaSumaMontoEImpuesto() {

        Mockito.when(impMock.calcular(amount)).thenReturn(16.0);

        double total = invoiceCalculatorRefac.calcularTotal(amount);

        assertEquals(116.0, total);
    ...
    @Test
    void calcularTotalConImpuestoRecargo_retornaSumaMontoEImpuesto() {
